...
#[,\!\?\.](?=[^’‘]*’[\s—-]) This regex worked, but cannot be implemented
...
Perhaps the most noted stylistic feature of *Absalom, Absalom!* is the length of the sentences. In fact, according to the 1983 *Guinness Book of World Records* it holds the dubious record of containing the longest sentence in the English language - 1,300 words (McWhirter 228). The claim is both absurd because it is an achievement that could be easily bested (PINKER, 83), and inaccurate because Faulkner himself beat his own "record" with a 1,600 word sentence in *Go Down, Moses* (Fargnoli 102). Faulkner's long sentence are more than a matter of trivia, however; they redefine the concept of a sentence altogether. Normally, computing sentence length in a text is a very simple procedure, because sentences are a regular structure in a text. With Absalom, there are so many parenthetical phrases and embedded citations that is difficult to judge whether a sentence has ended based on the punctuation. Nor does human reading of the individual "sentences" necessarily offer any clarity. It might be more accurate to say that it is a collection of words occassionally broken up by punctuation. On average, it there are roughly, `r round(absalom_sentences_mean$mean_length,0)` words with `r absalom_sentences_long` exceeding 100 words. THE LONG SENTENCE 
...
parentheses_text <-
  as.data.frame(str_extract_all(absalom_txt$text, "\\([^()]+\\)")[[1]])
colnames(parentheses_text) <- "text"
parentheses_text_count <- parentheses_text %>%
